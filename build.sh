...
UT_VERSION="protoc,secure_c,c_sec,eigen,protobuf_static_build,external_protobuf,nlohmann_json,\
          external_gtest,eigen_headers,ops_all_onnx_plugin_llt,opsplugin_llt,ops_fusion_pass_aicore_llt,ops_om_llt,\
          ...
          ops_cpp_om_utest,ops_benchmark"
}
...
set_env(){
  ...
  if [ ! "$usr_local" ];then
    ...
    echo  export PYTHONPATH=\$ASCEND_HOME/ops/built-in/op_impl/ai_core/tbe:\$ASCEND_HOME/compiler/python/site-packages:\$ASCEND_HOME/toolkit/python/site-package:\$PYTHONPATH >> ~/.bashrc
  fi
...
install_python_libs(){
  ...
  for p in numpy decorator sympy wheel psutil attrs jinja2
    do
...
usage() {
  ...
  echo "    --ops_om_llt build ops_om_llt"
  ...
  echo "    --ops_cpp_om_utest build ops_cpp_om_utest"
...
parse_changed_files() {
  ...
  if [[ "$UT_MODE" == "TRUE" ]]; then
      ...
      if [[ "$related_ut" =~ "OM_UT" ]];then
        logging "OM_UT is triggered!"
        OM_UT=TRUE
        EXEC_UT=TRUE
      fi
...
build_cann() {
  ...
  CMAKE_ARGS="$CMAKE_ARGS -DUT_NO_EXEC=$UT_NO_EXEC  \
            -DCPU_UT=$CPU_UT -DPASS_UT=$PASS_UT -DOM_UT=$OM_UT \
  ...
  if [[ "$UT_MODE" == "FALSE"  ]] && [[ "$CI_MODE" == "TRUE"  ]] && [[ ! "$related" =~ "OTHER_FILE" ]] ;then
      ...
      if [ "$UT_TEST_ALL" == "FALSE" -a "$CPU_UT" == "FALSE" \
            ...
            -a "$OM_UT" == "FALSE" -a "$ONNX_PLUGIN_UT" == "FALSE" ]; then
        CMAKE_ARGS="-DBUILD_PATH=$BUILD_PATH -DBUILD_OPEN_PROJECT=TRUE -DPRODUCT_SIDE=device -DBUILD_MODE=$build_mode -DWITH_CUSTOM_OP=$WITH_CUSTOM_OP"
...
main() {
  ...
  if [ "$create_lib_tag" ];then
    ...
    -DCPU_UT=$CPU_UT -DPASS_UT=$PASS_UT  -DOM_UT=$OM_UT -DTILING_UT=$TILING_UT\
  ...
  if [ "$CPU_UT" = "FALSE" -a "$PASS_UT" = "FALSE"  -a "$OM_UT" = "FALSE"\
