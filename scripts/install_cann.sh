...
extract_pack(){
  ...
  if [[ x"$install_type" == xdaily ]]; then
    ...
    tar -zxf  ascend_download/${file} -C ascend_download/out
  fi
}
...
install_Ascend(){
  ...
  opensdk=`ls ${folder} | grep "^CANN" | grep opensdk`
  if [[ x"$opensdk" == x ]];then
    echo "The opensdk package does not exist, please check."
  else
    if [[ $UID -eq 0 ]];then
        tar -zxf ${folder}/CANN-opensdk* -C  /usr/local/Ascend/latest/
    else
        tar -zxf ${folder}/CANN-opensdk* -C  ~/Ascend/latest
    fi
  fi
...
parse_args $@
get_arch
if [[ x"${install_type}" != x"local" ]];then
  test_network
  download_run
fi
bak_ori_Ascend
